...
parser.add_argument("--batch_size", type=int, default=100, help="Batch size for model evaluation")
parser.add_argument("--use_cache", type=bool, default=True, help="Whether to use cache during generation")
...
log("Using countdown reward function")
...
def save_model_checkpoint(model, tokenizer, iteration, model_name, initial_seed, args, dataset_size):
    ...
    log(f"Saving checkpoint at iteration {iteration} to {save_dir}...")
    model.save_pretrained(save_dir)
    tokenizer.save_pretrained(save_dir)
    log("Checkpoint saved successfully.")


def evaluate_model(model, tokenizer, input_text, target_text, accelerator,
                   seed_idx=None, thread_id=None, verbose=False, return_text=False,
                   batch_size=0, use_cache = True):
    """
    Generate a response from the model given an input (single or batch) and compute rewards.
    """
    if verbose:
        log(f"Process {accelerator.process_index} Thread {thread_id} evaluating seed {seed_idx}")
    ...
    all_generated_texts = []
    # Mini-batch generation to save memory
    for start in range(0, len(input_texts), batch_size):
        if STOP.is_set():
            return
        log(f"Process {accelerator.process_index} Thread {thread_id} processing batch from {start} to {min(start + batch_size, len(input_texts))} for seed {seed_idx}")
        ...
        for i in range(outputs.size(0)):
            ...
            try:
                ...
                toks = tokenizer.convert_ids_to_tokens(outputs[i], skip_special_tokens=True)
                toks = [t for t in toks if t is not None]
                gen = tokenizer.convert_tokens_to_string(toks)
            all_generated_texts.append(gen)

        del input_ids, attention_mask, outputs, tokenized
        torch.cuda.empty_cache()
    torch.cuda.empty_cache()

    # Compute rewards for batch texts
    rewards = []
    for i, (gen_text, tgt_text, inp_text) in enumerate(zip(all_generated_texts, target_texts, input_texts)):
        if STOP.is_set():
            return
        numbers = None
        target = None
    ...
    if return_text:
        return rewards, all_generated_texts
    else:
        return rewards
...
def process_seed(seed_args):
    ...
    if verbose:
        log(f"Process {accelerator.process_index} Thread {thread_id} processing seed {seed_idx} (value: {seed})")
    ...
    if verbose:
        log(f"Process {accelerator.process_index} Thread {thread_id} completed seed {seed_idx} with reward {average_reward:.4f}")
...
def main():
    ...
    if accelerator.is_main_process:
        log(f"Loaded {len(dataset)} countdown samples from {data_path}")

    if accelerator.is_main_process:
        log(f"Total processes: {accelerator.num_processes}, GPU threads per process: {args.gpu_threads}")
        log(f"Population size: {POPULATION_SIZE}, Iterations: {NUM_ITERATIONS}")
        log(f"Sigma: {SIGMA}, Alpha: {ALPHA}")
        log(f"Using batch size: {args.batch_size} for model evaluation")
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        if args.verbose:
            log(f"Process {accelerator.process_index} starting iteration {iteration + 1}/{NUM_ITERATIONS}")

        # Generate seeds on main process only
        if accelerator.is_main_process:
            if args.verbose:
                log(f"Main process {accelerator.process_index} generating seeds")
            ...
            if args.verbose:
                log(f"Worker process {accelerator.process_index} waiting for seeds")
        ...
        if args.verbose:
            log(f"Process {accelerator.process_index} received seeds")
        ...
        if args.verbose:
            log(f"Process {accelerator.process_index} assigned {len(local_seeds)} seeds: {[idx for idx, _ in local_seeds]}")

        # Process seeds in smaller batches to reduce memory pressure
        local_rewards = []
        batch_size = max(1, min(args.gpu_threads, len(local_seeds)))
        log(f"Process {accelerator.process_index} thread batch size {batch_size}")
        ...
        if args.verbose:
            log(f"Process {accelerator.process_index} updating model weights")
        ...
        if accelerator.is_main_process:
            log(f"Iteration {iteration + 1}/{NUM_ITERATIONS}, Time: {iter_time:.2f}s, Mean: {mean_reward:.2f}, Min: {min_reward:.2f}, Max: {max_reward:.2f}")
            log(f"GPU Memory: {torch.cuda.memory_allocated() / 1024**2:.2f}MB allocated, {torch.cuda.max_memory_allocated() / 1024**2:.2f}MB peak")
    ...
    if accelerator.is_main_process:
        ...
        log(f"Saving final model to {save_dir}...")
        original_model.save_pretrained(save_dir)
        tokenizer.save_pretrained(save_dir)
        log("Final model saved successfully.")
